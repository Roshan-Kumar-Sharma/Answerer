...
async function fetchData() {
    try {
        ...
        const dbResponse = await fetch(
            `${location.origin}/api/v1/posts/fetch?sub=${sub}&&topic=${topic}&&page=${currentPageNumber}&&limit=${pageLimit}&&answered=false`
        );
...
async function gotoPage(pageNumber) {
    ...
    const dbResponse = await fetch(
        `${location.origin}/api/v1/posts/fetch?sub=${sub}&&topic=${topic}&&page=${pageNumber}&&limit=${pageLimit}&&answered=false`
    );
